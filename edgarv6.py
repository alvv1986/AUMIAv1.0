...
#plt.title("ANTHROPOGENIC SOURCES", loc='left', fontsize=12)
cbar = m.colorbar(cs, location='bottom', ax=ax, pad="2%", ticks=levels[::6])
cbar.set_label("$Kg$ $m^{-2}s^{-1}$ [$x10^{-10}$]", labelpad=2, fontsize=15)
cbar.ax.tick_params(labelsize=15) 

plt.savefig("edgar_ch4_total_no_fires.png",bbox_inches='tight')
plt.show()
